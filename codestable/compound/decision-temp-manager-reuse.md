...
需要持久化的输出先使用 `createStaging()` 获取 reservation 和 heartbeat lease，成功后再原子 finalize；失败路径必须 discard staging 并释放 reservation。当前 M2 仍在接通 staging writer，现有 `paging.ts` 继续使用 legacy 文本 cache。
