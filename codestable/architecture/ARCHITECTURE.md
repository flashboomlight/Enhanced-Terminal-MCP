...
> 最后核对：2026-08-19（merge-e-hardening-base 合入 E hardening 线模块后并集对齐；最终重写归 M4）
...
| `src/tools/command.ts` | `execute_command` / `batch_execute` / `watch_command`。Windows 消费 `shell.ts` 统一解析的 shell spec（默认 pwsh 7，详见 ADR-7/14），Unix 用 `/bin/sh -c`；命令 policy + 危险模式检查 + 限流 + SafeGuard + `spawnStream` + audit |
...
| `src/temp-manager.ts` | 临时目录 TTL + LRU 回收 |
| `src/paging.ts` | 命令输出分页存储与读取 |
...
- **ADR-8 流式执行**：命令执行统一 `spawnStream`，输出超上限截断并终止；超时先 SIGTERM，2s 后 SIGKILL。
...
- **ADR-11 Windows 搜索捆绑 Everything**：`es_tool/es.exe` 随包分发（M3 将改为用户本地可选提供），SHA-256 锁定校验，10s 超时兜底原生递归搜索。
