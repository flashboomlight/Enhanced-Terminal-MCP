...
- Production conformance gate: `pnpm run gate` is now the canonical release gate and runs build, type-check, lint, full tests, main/tools coverage, latency, production dependency audit, package verification, an actual pack, and clean npm consumer startup. The same script accepts `--ci` so CI can keep the existing latency advisory policy without duplicating the gate command chain.
