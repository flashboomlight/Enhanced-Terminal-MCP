...
- Bilingual reference docs: the five `docs/` pages (installation, configuration, tools, safety, troubleshooting) now have Chinese twins (`docs/*.zh-CN.md`), each reachable via a language switcher at the top of its English page and cross-linked to the Chinese counterparts; `README.zh-CN.md`'s doc map and body links now point to the Chinese pages. English remains the source of truth (stated on each Chinese page). The local docs verifier now checks all six bilingual pairs for heading-structure parity, zh-CN coverage completeness, and tool-contract parity in both languages.
- Pre-public polish: a prominent warning block at the top of both READMEs now states the execution boundary up front — full shell strings run on the host with your user privileges, the safety layers are defense in depth (not a sandbox), and the server must not be exposed to untrusted clients or networks. `THIRD_PARTY_NOTICES.md` now records the pinned `@modelcontextprotocol/sdk@1.29.0` license line explicitly (MIT, Copyright (c) 2024 Anthropic, PBC) and notes the upstream MIT → Apache-2.0 transition so the license is re-checked on any SDK upgrade.
...
- Linux parity: README gained a `Linux Notes` section (shell, `zip`/`unzip` archive dependency, search engines) and CI now runs the unit suite + both coverage gates on Ubuntu, with platform-aware coverage thresholds and a best-of-3 latency sampler on non-Windows.
...
- Added real stdio MCP conformance checks, a bounded hostile-input corpus, and cross-platform smoke checks for Windows/Linux/macOS and Node 22/24. Gate reports are written under `.etmcp/gate-report.json` without retaining full command output.
