...
| 12 | security-and-mcp-conformance-gates | done | working tree | canonical CI gate、conformance、hostile-input、platform smoke、release evidence |
| 13 | docs-and-architecture-closeout | planned（等 #12） | — | DOC-01（v3.1/28 残留文字统一等） |

各条目的设计/checklist/acceptance 三件套在 `codestable/features/YYYY-MM-DD-{slug}/`；审计文档 §6.1–§6.12 有每条的实施状态回写。
...
1. **#13 `docs-and-architecture-closeout`**（#12 已完成）：统一 v4.0.0/27/26 的全部残留文字（usage-guide "NEW in v3.1" 段、CHANGELOG 历史边界、SECURITY/依赖维护入口）、现状档案回写。
...
- **全量测试高负载 flake**：#12 已将 lock-lease heartbeat 改为串行续租、测试改为等待可观察 heartbeat，并为 Windows staging rename 增加有界 EPERM/EBUSY/EACCES 退避；lock/paging 定向各 5 次与 release gate 均通过，后续仍需 CI runner 观察矩阵稳定性。
