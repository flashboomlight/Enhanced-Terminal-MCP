...
- 三个命令工具已接入 `src/command-output.ts` 的共享原始字节捕获、actual 计数、流式 secret matcher、staging spill、page cache v2 与公开 A+ envelope（M2 验收时阶段 C 门禁全绿）；M3 Everything 可选解析、搜索契约和 npm 发布裁剪已验收，解析顺序为 `ENHANCED_TERMINAL_ES_PATH` → state 目录 → unavailable，运行期不下载。非 Windows 的 `search_files` 另有可选 fd/fdfind 引擎加速（`src/fd-resolver.ts`：`ENHANCED_TERMINAL_FD_PATH` 显式 fail-closed → PATH `fd`/`fdfind` → 隐式不可用静默 native 兜底；进程级缓存；详见 `codestable/features/2026-08-29-linux-fd-search/`）。
...
- 测试策略：工具行为主要由 `tests/e2e-latency.test.ts` 子进程 e2e 覆盖；coverage 主配置排除 `src/index.ts`、`src/tools/**`、测试源码和 `tests/`，因为 V8 不能收集子进程覆盖率；工具纯逻辑由 `tests/unit/tools/` 覆盖（files/manage/system/archive 有专属单测），工具层另有专属覆盖率底线 `pnpm run test:coverage:tools`。coverage 运行还跳过延迟基准文件，避免插桩开销造成假失败。非 Windows 主 coverage 阈值平台化（`vitest.config.ts`，Windows 80/80/70/80，非 Windows 81/81/70/78）；latency 断言用 best-of-3 采样防抖。CI 见 `.github/workflows/ci.yml`（ubuntu lint/tsc + ubuntu Node 22 单测与双覆盖门禁；windows Node 22/24 跑 build/test/覆盖门禁，latency 非阻塞）。
...
- 开发依赖安装统一使用 pnpm。pnpm 的共享 content store 由机器配置决定（用 `pnpm store path` 查询本机实际路径）；机器路径不得写入实现文件、package metadata、lockfile 或发布物。每个项目仍保留自己的 `node_modules` 和 virtual store，不共享运行时目录。
- Windows 构建、测试和相关临时数据优先使用项目内 Git 忽略的目录，例如 `.etmcp/test-tmp` 与 `.etmcp/npm-cache`；不得落到系统盘（C 盘）。若工具依赖 `TEMP` / `TMP` / `TMPDIR`，运行命令时将它们显式指向项目内临时目录；`.etmcp/npm-cache` 仅用于 npm clean consumer 验证
