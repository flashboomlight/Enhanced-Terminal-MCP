...
Supports **26 tools** across 7 categories: command execution, file I/O, system management, search, archives, telemetry, and session management.

## Features

- **3-Level Safety System** — strict/normal/off via `MCP_SAFETY_MODE`, 19 dangerous command patterns blocked, critical process protection
...
- **Session Persistence** — cwd, env vars, and command history survive restarts (auto-saved to temp dir)
...
|------|-------------|--------|
| `execute_command` | Execute a single shell command with timeout, exit code, and streaming output | destructive |
...
| Tool | Description |
|------|-------------|
| `telemetry_report` | Tool call metrics: latency, errors, cache hit rates |
...
### Resources
- `health://status` — JSON health check with version, metrics, cache, session info
